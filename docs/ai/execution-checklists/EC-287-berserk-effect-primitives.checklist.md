...
- **`bind`/`ref` binding pair (locked):** `bind?: string` on `MoveCardNode` (writes the moved card id) and `CardReference { ref: string }` (reads it) are the two halves of the D-24029 §9 context mechanism. `CardReference` is the ONLY shape `CardPrintedStatExpression.card` accepts this WP (no selector/literal card forms). Both key into the `EffectExecutionContext`.
...
- **Parser seam (locked):** in the `[keyword:X]` handling, when `normalizedKeyword ∈ HERO_COMPOSITION_MARKERS`, push **`structuredClone(HERO_COMPOSITION_MARKERS[name])`** of that AST — NEVER the shared registry object — onto a `primitiveEffects` accumulator. **`structuredClone` is mandated** (not a hand-written literal): it is a Node v22 global, deep-copies deterministically over the plain-JSON AST (no functions/`Map`/`Set` in a descriptor), and removes both the aliasing footgun and the literal-drift risk — do **NOT** push to `hook.keywords` (`berserk` is not a `HeroKeyword`; `isValidHeroKeyword('berserk')` stays false). The copy is mandatory: a hook that aliased the module-level registry AST would let one match mutate the shared constant (D-13502 freshly-constructed-hook discipline). Assemble `hook.primitiveEffects` in `buildHeroAbilityHooks` only when non-empty (mirror the `hook.effects`/`hook.conditions` conditional-construction guards — `exactOptionalPropertyTypes`: assign the field only when present, never `: x ?? undefined`).
