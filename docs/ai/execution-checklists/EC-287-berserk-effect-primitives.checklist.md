...
- Unknown node type / value-expression type / missing `ref` / missing `cardStats` → warn to `G.messages` + safe default (skip / 0), **never throw**.
...
- A second composition marker / a general markup-AST parser appeared → premature abstraction (D-24029 §10); scope is `berserk` only + the cousin test.
