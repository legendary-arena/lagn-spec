...
| `hero_count_lookup` | `cardCounts[card.rarity]`; rarity-key only; no fallback |
...
2. **Hero: rarity-key** — `cardCounts[card.rarity]` only; no fallback
...
- **Wrong rarity key:** cardCounts keyed by name, not rarity → verify `cardCounts[card.rarity]`
