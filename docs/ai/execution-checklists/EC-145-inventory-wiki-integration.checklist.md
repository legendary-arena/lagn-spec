...
- **Visible-failure invariant under non-gating:** `continue-on-error: true` lets the workflow proceed past a script crash, but the step itself **must remain visibly failed** in the GitHub UI (red ❌ on the step, stderr captured to the run log). Wrapping the script invocation in `|| true` (or any other shell trick that swallows the non-zero exit code) is forbidden — that turns the step silently green and discards the script's "not a gate, but loud about it" header contract. Verify by triggering an intentional crash via `workflow_dispatch` against a deliberately broken commit; the step must show red and the run log must capture the script's stderr.
