...
- §18 prose-vs-grep: the rewritten comments must NOT contain literal `boardgame.io` / `Math.random` / `.reduce(` — the After-Completing greps expect no output; cite the rule instead (reword the current line-14 `No .reduce().` + line-278 `boardgame.io's PRNG` comments). Precedent WP-037.
