...
- `display` field resolved via `resolveDisplay(G.pendingHeroChoice.cardId, gameState)` — fresh shallow copy
...
- Component: `PendingHeroChoicePrompt.vue` — `defineComponent` per D-6512; NOT a modal; NOT dismissible
- Prompt visible only when `pendingHeroChoice` defined AND viewer is the choosing player
...
- `apps/arena-client/src/components/play/PendingHeroChoicePrompt.test.ts` — **new** — ≥5 tests
- `apps/arena-client/src/components/play/TurnActionBar.test.ts` — **modified** — ≥2 disable tests

## After Completing

- [ ] `pnpm --filter @legendary-arena/game-engine test` exits 0, count ≥ **1169**
- [ ] `pnpm --filter arena-client test` exits 0, count ≥ **495**
