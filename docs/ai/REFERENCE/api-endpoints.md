...
(e.g., `Pending: WP-115 (STUB DRAFT 2026-04-29)` → `Wired` when WP-115
executes) are full-row replacements, never field-level edits. Enforcement
...
| `Library-only` | (n/a) | (n/a — function `getScenarioLeaderboard`) | (n/a — caller-injected `DatabaseClient` + optional `LeaderboardDependencies`) | `(options: LeaderboardQueryOptions, database, deps?: LeaderboardDependencies)` per [`apps/server/src/leaderboards/leaderboard.logic.ts:151`](../../../apps/server/src/leaderboards/leaderboard.logic.ts) | `Promise<ScenarioLeaderboard>` per WP-054 | WP-054 (public leaderboards read-only) | **Route-less at WP-054 close per §Lifecycle Prohibition;** WP-115 wires the HTTP surface and graduates this row to `Wired` per the catalog footer single-row-graduation model. PAR fail-closed via `PRODUCTION_DEPENDENCIES.checkParPublished = () => null` until the wiring caller injects a bound `parGate.checkParPublished`. |
| `Library-only` | (n/a) | (n/a — function `getPublicScoreByReplayHash`) | (n/a — caller-injected `DatabaseClient`) | `(replayHash: string, database)` per [`apps/server/src/leaderboards/leaderboard.logic.ts:238`](../../../apps/server/src/leaderboards/leaderboard.logic.ts) | `Promise<PublicLeaderboardEntry \| null>` per WP-054 | WP-054 (public leaderboards read-only) | **Route-less at WP-054 close.** Returns `null` on miss; WP-115 translates `null → 404` at the HTTP boundary. Sensitive fields (`accountId`, `submissionId`, `email`, `authProvider`, `authProviderId`, `stateHash`, `scoreBreakdown`) are stripped from `PublicLeaderboardEntry` per D-5201; never re-introduced at the route layer. |
| `Library-only` | (n/a) | (n/a — function `listScenarioKeys`) | (n/a — caller-injected `DatabaseClient`) | `(database)` per [`apps/server/src/leaderboards/leaderboard.logic.ts:281`](../../../apps/server/src/leaderboards/leaderboard.logic.ts) | `Promise<readonly string[]>` per WP-054 | WP-054 (public leaderboards read-only) | **Route-less at WP-054 close.** Discoverability seam — returns the deduplicated set of `scenario_key` values present in `legendary.competitive_scores` so future leaderboard UIs (and WP-115's `GET /api/leaderboards/scenarios` endpoint) can enumerate available leaderboards without prior registry knowledge. |
...
### Pending: WP-115 (STUB DRAFT 2026-04-29)

Three public leaderboard endpoints drafted in
[`docs/ai/work-packets/WP-115-public-leaderboard-http-endpoints.md`](../work-packets/WP-115-public-leaderboard-http-endpoints.md)
(committed at `bfdefe1` 2026-04-30 as a stub draft). Auth posture is
`guest` per WP-115 §Vision Alignment "anonymous access". When WP-115
executes, its commit replaces these rows wholesale per D-11804.

| Status | Method | Path | Auth | Request Schema (file ref) | Response Schema (file ref) | Authorizing WP | Notes |
|---|---|---|---|---|---|---|---|
| `Pending: WP-115 (STUB DRAFT 2026-04-29)` | `GET` | `/api/leaderboards/scenarios` | `guest` | (none — empty body) | per WP-115 §Goal lines 99-101 (scenario index) | WP-115 (public leaderboard endpoints) | Forward-link: [`docs/ai/work-packets/WP-115-public-leaderboard-http-endpoints.md`](../work-packets/WP-115-public-leaderboard-http-endpoints.md). Returns the curated list of scenarios for which leaderboard rows exist. Replaced wholesale when WP-115 executes (status → `Wired`, schema file refs filled in). |
| `Pending: WP-115 (STUB DRAFT 2026-04-29)` | `GET` | `/api/leaderboards/scenarios/:scenarioKey` | `guest` | `:scenarioKey` path param | per WP-115 §Goal lines 99-101 + §306 (per-scenario score list) | WP-115 (public leaderboard endpoints) | Forward-link: [`docs/ai/work-packets/WP-115-public-leaderboard-http-endpoints.md`](../work-packets/WP-115-public-leaderboard-http-endpoints.md). Returns the score list for the named scenario. Replaced wholesale when WP-115 executes. |
| `Pending: WP-115 (STUB DRAFT 2026-04-29)` | `GET` | `/api/leaderboards/scores/:replayHash` | `guest` | `:replayHash` path param (verbatim spelling per `00.2`) | per WP-115 §Goal lines 99-101 + §470 (single-score detail with replay link) | WP-115 (public leaderboard endpoints) | Forward-link: [`docs/ai/work-packets/WP-115-public-leaderboard-http-endpoints.md`](../work-packets/WP-115-public-leaderboard-http-endpoints.md). Returns the single-score detail for the named replay. Replaced wholesale when WP-115 executes. |
