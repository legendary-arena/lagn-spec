...
**Version:** 1.1 (refreshed 2026-05-02 post-WP-115 closeout — see §v1.1 Update Log)
...
## v1.1 Update Log (2026-05-02)

Refresh covering knowledge gained from WP-115 / EC-119 closeout (Commit A `ab9bcce`, 2026-05-01) and downstream catalog state. Substantive changes since v1.0 (2026-04-30):

1. **WP-115 has shipped — leaderboard HTTP surface is `Wired`, `pg.Pool` lifecycle exists.** WP-115 / EC-119 closed at commit `ab9bcce` on 2026-05-01 with three new HTTP endpoints under `/api/leaderboards/*` and the long-lived `pg.Pool` singleton at `apps/server/src/db/database.ts`. `apps/server/src/server.mjs` and `apps/server/src/index.mjs` were modified (Pool construct + register routes + SIGTERM closePool). **Impact on WP-116: zero** — disconnect/reconnect policy is governance-only, lives in `docs/**` + `.claude/rules/architecture.md`, and does not intersect WP-115's read-only HTTP surface or the pool lifecycle. The `## Context (Read First)` anchor at `apps/server/src/server.mjs` is still accurate (no reconnect handler shipped at WP-115 close — Pool construction + route register + SIGTERM closePool are all the new wiring; the SIGTERM path closes the pool after `httpServer.close()` resolves but does NOT implement player-disconnect semantics, which remains undefined and is exactly what this WP locks).
2. **D-11501..D-11506 now occupy the slot range immediately preceding D-11601..D-11606.** WP-115's six DECISIONS entries (Pool location / Pool sizing / rate-limit deferral / `Cache-Control: no-store` v1 lock / D-10202 reaffirmation / Pool-log verbatim) are continuous from D-11501 through D-11506. WP-116 picks up cleanly at D-11601 with no slot collision risk. The D-11506 verbatim-text-lock pattern is a new precedent (alongside D-11804 closed-set + replace-whole-row semantics) for "lock the literal value of a string that downstream tools will grep" — referenced where applicable below.
3. **D-11604 closed-set discriminator pattern has a stronger precedent.** D-11604's `endReason: 'abandoned'` lock follows the WP-118 D-11804 status-enum closed-set pattern (lock field-presence + one canonical value, defer the full enum to the implementation surface). WP-115's execution applied D-11804 at scale — three `Library-only → Wired` row transitions executed wholesale per D-11804 replace-whole-row merge semantics (`docs/ai/REFERENCE/api-endpoints.md` net delta −3 placeholder / −3 Library-only / +3 Wired at commit `ab9bcce`). The D-11604 pattern is now a settled precedent, not a forward-projection.
4. **WP-115 D-11505 reinforces the "deferral despite available infrastructure" pattern WP-116's future implementation WP will inherit.** D-11505 reaffirmed D-10202 (WP-102 profile-route wiring stays deferred even though the long-lived `pg.Pool` introduced by WP-115 is exactly the lifecycle anchor WP-102's `registerProfileRoutes` was waiting on). WP-116's future implementation WP will face the same posture — WP-090's transport ships, WP-116 locks policy, but a discrete *implementation* WP owns the actual `apps/server/src/server.mjs` reconnect-handler wiring with its own commit, governance close, and post-mortem. The D-11505 precedent normalizes that separation.
5. **§Out of Scope unrelated-untracked-files protocol updated.** The original v1.0 line referenced "a residual `EC-119-public-leaderboard-http-endpoints.checklist.md` from WP-115 stub work" as an example untracked file. WP-115 / EC-119 has shipped and the EC-119 checklist is no longer "stub work" — it's a governance artifact preserved for regression reference per `EC_INDEX.md`. The protocol itself (do-not-stage unrelated untracked files; no `git add .` / `-A` / `-u`) is unchanged; only the stale example is refreshed below.
6. **Test baselines remain irrelevant to WP-116** because the WP is governance-only (zero `apps/` / `packages/` files staged; no test runner involvement). For reference (so future readers don't re-derive): post-WP-115 baselines are server `pass 56 / fail 0 / skipped 32` and engine `pass 604 / fail 0`. WP-116 leaves both untouched by construction.
7. **Session-Context multiplayer-feature roster updated.** The original Session Context line "further multiplayer features (team play WP-109, leaderboard WP-115, future ranked play)" listed WP-115 as a forward-looking multiplayer feature that would need disconnect policy. WP-115 actually shipped as **read-only HTTP** over verified replay records — it does not re-litigate disconnect policy because it is not a live-match surface. The roster is corrected below: team play WP-109 (still BLOCKED on WP-104) and future ranked-play / spectator WPs are the real consumers of WP-116's policy.
...
WP-090 shipped browser-to-engine live-match wiring over Socket.IO 4.8.x via `boardgame.io/client`, but the architecture doc and `.claude/rules/*` are silent on what the *application* does when a player drops mid-match — turn-handover, lobby readiness on rejoin, abandonment policy, and replay-on-abort behavior are all undefined. boardgame.io provides reconnect plumbing; this packet writes the application-level policy on top of it before further multiplayer features (team play WP-109, future ranked play, future spectator surface) re-litigate the same questions.

> **Note (refreshed 2026-05-02):** WP-115 (Public Leaderboard HTTP Endpoints, closed 2026-05-01 at `ab9bcce`) is **not** a live-match surface — it ships read-only HTTP over verified replay records and does not need disconnect policy. The original v1.0 Session Context listed WP-115 as a forward-looking multiplayer feature; that was a forecasting error caught at v1.1 refresh. The leaderboard surface is stateless, anonymous, and `Cache-Control: no-store`-locked under D-11504 — it has no live-match concept to reconcile against this WP's policy.
...
- `apps/server/src/server.mjs` — read the current Server() wiring; confirm there is no existing reconnect handler so the policy does not collide with shipped code. **Refreshed 2026-05-02 post-WP-115 closeout (commit `ab9bcce`):** the file now constructs a long-lived `pg.Pool` (D-11501 / D-11502), logs the locked construction message `'[server] pg.Pool constructed (max=10)'` (D-11506 verbatim lock), and registers `registerLeaderboardRoutes(server.router, pool, parGate)`. None of those touch reconnect semantics — the SIGTERM path in `index.mjs` closes the pool *after* `httpServer.close()` resolves but does **not** implement any player-disconnect handling. WP-116's policy still has a clean canvas.
...
**Session protocol — unrelated untracked files (per pre-flight 2026-04-30 PS-5, mirroring WP-030 / D-3001 / WP-117 §Out-of-Scope / WP-118 §5.2 mystery-untracked-file precedent):** the execution session may observe unrelated untracked files in `git status` (e.g., a gitignored-by-spirit `docs/ai/invocations/copilot-check-wp115.md` scratchpad left over from WP-115 v1.1 SPEC prep, future arch-inventory regenerations under `docs/ai/audits/`, future per-WP scratchpads under `docs/ai/invocations/`). These are out of WP-116 scope. Do not stage, modify, or comment on them. Stage by exact filename only — never `git add .` / `-A` / `-u`. The WP-116 close-out commit must contain only the resolved-allowlist file diffs and nothing else. The same rule applies to **this prep commit** — only `docs/ai/work-packets/WP-116-disconnect-reconnect-semantics.md` is staged. (v1.0's example referenced `EC-119-public-leaderboard-http-endpoints.checklist.md` as residual stub work; that EC has shipped at commit `ab9bcce` 2026-05-01 and is now a regression-reference governance artifact, not stub work — the example is refreshed at v1.1.)
