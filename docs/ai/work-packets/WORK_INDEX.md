...
- [ ] WP-180 — Build-Time Version Stamping (Cross-App). **Pending**. Injects `__APP_VERSION__` + `__BUILD_TIMESTAMP__` + `__GIT_SHA__` into every Vite app via `vite.config.ts` `define`; adds a fixed-position `<VersionBadge>` footer component to arena-client, registry-viewer, dashboard, legends-board; adds `GET /api/version` endpoint to server. Solves the "worktree updated but Cloudflare serving stale code" visibility gap. No engine/registry/preplan changes. No deps (parallel-safe). See [WP-180](WP-180-build-version-stamping.md).
