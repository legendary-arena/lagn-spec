...
- [ ] WP-188 — Villain & Henchman Escape/Overrun Effect-Marker Enrichment (Card Data) (EC-215). Card-data tooling only (`scripts/convert-cards/`, `data/cards/`); no engine/registry/server code. **Continuation of WP-187 + upstream prerequisite for WP-186** — authors the `[effect:]` markers on the curatable `Escape:`/`Overrun:` ability lines that WP-186's parser reads. Single behavioral script change: widen `apply-effect-markers.mjs` `SUPPORTED_TIMINGS` from `['ambush','fight']` → `['ambush','fight','escape','overrun']` (the matching code is already timing-generic), plus curated `escape?`/`overrun?` entries in `villain-effect-markers.json` + docstring touch-ups. **Yield is small and lopsided:** escape effects are overwhelmingly *each-player* ("Each player KOs one of their Heroes" ×9), but the MVP vocabulary is *current-player*-biased with no each-player KO keyword — so the dominant escape KO pattern goes to `_unassigned` (`reason: no-vocabulary-keyword`, D-18802) pending a future `koHeroEachPlayer` vocab expansion. The curatable subset is dominated by `gainWoundEachPlayer` (unconditional "Escape: Each player gains a Wound." lines) — **the first data that keyword ever gets** (zero under WP-187, D-18803). No new keyword; vocabulary stays at 5. No `.test.ts` (ops-script convention). Hard-dep: WP-187 ✅ (the overlay it extends). D-18801..D-18803 (proposed). 1 modified script + 1 modified map + bounded `data/cards/*.json` regen + governance. See [WP-188](WP-188-villain-escape-effect-marker-enrichment.md) + [EC-215](../execution-checklists/EC-215-villain-escape-effect-marker-enrichment.checklist.md).
