...
- [ ] WP-190 — Villain Each-Player-KO Effect-Marker Curation (Card Data) (EC-217). Card-data tooling only (`scripts/convert-cards/`, `data/cards/`); no engine code. **Data half of the `koHeroEachPlayer` expansion** — authors `[effect:koHeroEachPlayer]` markers on the ~11 unconditional magnitude-1 "Each player KOs one Hero" lines across `Ambush:`/`Fight:`/`Escape:` (villain per-card + henchman group-level), promoting them out of WP-187/188 `_unassigned`. Appends `koHeroEachPlayer` to the overlay's local keyword array (5→6, position 6, byte-matching WP-189) + adds an each-player-KO `--propose` heuristic (the existing `koHeroCurrentPlayer` heuristic over-captures each-player lines). KO-only: magnitude>1 / filtered (cost/non-grey/team) / conditional / choice variants stay `_unassigned`; each-player ≠ current-player (no re-marking `koHeroCurrentPlayer` lines). Master Strike each-player-KO is the WP-024 system, out of scope. No `.test.ts` (ops-script convention; loud-fail + idempotency). Hard-deps: WP-189 ✅ (engine keyword), WP-187 ✅ (overlay), WP-188 ✅ (escape timings). D-19001..D-19002 (proposed). 1 modified script + 1 modified map + bounded `data/cards/*.json` regen + governance. See [WP-190](WP-190-villain-each-player-ko-marker-curation.md) + [EC-217](../execution-checklists/EC-217-villain-each-player-ko-marker-curation.checklist.md).
