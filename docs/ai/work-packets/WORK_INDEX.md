...
- [ ] WP-180 — Build-Time Version Stamping (Cross-App). **Pending**. Injects `__APP_VERSION__` + `__BUILD_TIMESTAMP__` + `__GIT_SHA__` into every Vite app via `vite.config.ts` `define` (git SHA with try/catch fallback to `'unknown'`); adds a fixed-position `<VersionBadge>` footer component (byte-identical template+style across all four apps) to arena-client, registry-viewer, dashboard, legends-board; adds `GET /api/version` endpoint to server (process-start timestamp, not build time — D-18001). Solves the "worktree updated but Cloudflare serving stale code" visibility gap. Per-app component over shared `packages/ui` package (D-18002 — deferred until a second shared UI component justifies the package overhead). No engine/registry/preplan changes. No deps (parallel-safe). EC-204. See [WP-180](WP-180-build-version-stamping.md).
