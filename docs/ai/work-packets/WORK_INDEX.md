...
- [ ] WP-163 — Autoplay Playback Controls (Server) — pending (drafted 2026-05-19, baseline `7b0f944`; pre-flight READY, copilot PASS). Server half of media-player playback for "Watch Bot Play": new pure helper `apps/server/src/autoplay/playbackController.mjs` (cursor-based snapshot history, `maxHistory=100`, single-consumer pause gate) + six bodyless REST endpoints `POST /api/match/autoplay/:matchId/{pause,resume,step-forward,step-back,restart,go-to-end}` returning a standardized `{ ok, paused, historyLength, cursor, mode, uiState?, error? }` envelope + `runBotMatch` integration (initial + per-move `pushState`, `waitIfPaused()` gate, `getActiveDelay()` substitution, exit-path cleanup). Rewind is REST-only and visual-only — no `boardgame.io` mutation, no persistence (buffer = Class 1 Runtime State per D-16306). Six new whole API-catalog rows (D-11804, `Status: Wired`, `Auth: guest`). Paired with WP-164 (client), which hard-deps these endpoints landing on `main` first. Hard-deps: WP-090, WP-118 — both ✅. D-16301..D-16309. 8 files (2 new + 1 modified source + 5 governance). See [WP-163](WP-163-autoplay-playback-server.md) + [EC-180](../execution-checklists/EC-180-autoplay-playback-server.checklist.md).
