...
- [ ] WP-158 — Complete-Game Regression Tests (Seed-Faithful Fixture Harness) — **pending**. Engine-only regression net under `packages/game-engine/src/test/fixtures/`. Adds a seed-faithful mulberry32 fixture harness (driver, hash helper, schema), a CLI recorder at `scripts/record-game-fixture.mjs`, one sentinel fixture proving the pipeline, and operator docs at `docs/ai/REFERENCE/complete-game-tests.md`. Does NOT modify `packages/game-engine/src/replay/**` (D-0205 stands) or `packages/game-engine/src/simulation/**` (WP-036 Scope Lock — mulberry32 + MOVE_MAP precedent mirrored locally). Three-tier oracles: `outcome` → `messages` → `finalStateHash`. Hard-deps: WP-013 (snapshots), WP-027 (replay primitives), WP-036 (simulation precedent), WP-079 (D-0205 narrowing), WP-080 (`applyReplayStep` precedent) — all complete. 01.5 NOT INVOKED. Parallel-safe with every other Phase 7 WP. D-15801. See [WP-158](WP-158-complete-game-regression-tests.md). ⚠️ Needs review
