...
- [ ] **(deferred placeholder) Phase 1b** — Per-set patch curation across the remaining ~24 split-side heroes (`bkwd` other splits not in canonical reference patch, `mgtg`, `msis`, `msmc`, `msp1`, `wpnx`, `wwhk`, `xmen`). Drafted after WP-138 Phase 1a lands; consumes the 262-entry audit-warning worklist surfaced by this run. Phase 1b WP must distinguish true split pairs (get explicit `physicalCards[]` block) from false positives (declarative annotation suppressing the warning). Hard-dep on Phase 1a; Phase 2 cannot start until Phase 1b lands every per-set patch.

- [ ] **(deferred placeholder) Phase 2** — Engine consumer migration (`buildHeroDeck.ts` reads `physicalCards[].count` for deck reservoir; `buildCardDisplayData.ts` reads `physicalCards[].imageUrl`) + viewer migration in `apps/registry-viewer/src/`. Hard-dep on Phase 1b. 01.5 likely INVOKED — deck-reservoir size for split heroes changes; `computeStateHash` inputs via `G.cardStats` / `G.cardDisplayData` may shift; cascade procedure per WP-135 / WP-137 template.

- [ ] **(deferred placeholder) Phase 3** — `HeroCardSchema.imageUrl` removal + R2 image migration (rename split-pair files to combined-name pattern; delete legacy per-side hero card image files) + final regeneration of `data/cards/*.json` without per-side imageUrl. Hard-dep on Phase 2's grep gate clearing `card.imageUrl` in src.
