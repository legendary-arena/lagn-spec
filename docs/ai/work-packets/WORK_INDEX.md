...
- [ ] WP-196 — Dashboard Net Revenue + Paid-Action Errors Widgets. **Draft** 2026-05-31. Client-only (`apps/dashboard/`) follow-up to the WP-157 scaffold + WP-162 polish. Implements **WP-A** of the operator-dashboard pre-mortem grouping (drafted 2026-05-31; B = acquisition funnel, C = retention cohorts, D = public-surface health + general error monitor + cost watchdog, E = TAM-saturation + content-breadth — all not yet drafted). Adds two widgets on `/monetization`: **NetRevenueChartWidget** — stacked-bar 4-bucket decomposition (gross − royalty − Stripe fees − infra COGS) with `revenueDeductions.ts` static config (royalty/fees/COGS percentages ship as flagged-mock placeholders pending finance review); integer-cents arithmetic throughout (no floating-point dollars); negative-net days preserved, not clamped (negative is a real signal). **PaidActionErrorsWidget** — surfaces two billing-side health signals: Stripe webhook fulfillment failure rate (`legendary.stripe_events.process_error IS NOT NULL`) and Checkout intent abandonment rate (`legendary.stripe_checkout_sessions.intent_status IN ('expired', 'canceled')`); future server endpoint path `/metrics/billing/health` locked here so the follow-up server WP has no naming ambiguity. Both widgets follow the WP-157 Widget Contract (4-state rendering + freshness badge showing `MOCK`) and the WP-162 design-token / theme-toggle discipline (subscribe to `dashboard-theme-change`; re-resolve echarts color tokens on the next paint). All data mock (`VITE_USE_MOCKS=true`); no server endpoints added; no schema changes; no `apps/server/` files touched; zero `@legendary-arena/*` imports. §17 PASS (Financial Sustainability + NG-1..NG-8 — internal admin/finance operator surface, no player-facing copy). §20 N/A with justification (analytical / retrospective mention carve-out — operator widget, not a funding affordance). §21 N/A with justification (no `apps/server` HTTP endpoint or library function added; client-side reference to future `/metrics/billing/health` follows the existing `fetchRevenueHistory` precedent). D-19601..D-19603 reserved (4-bucket model + integer-cents discipline, placeholder royalty % flagged MOCK, paid-action error union + locked endpoint path). Hard-deps: WP-157 ✅, WP-162 ✅. ~5 new + ~4 modified files (single-layer Client work). EC TBD at execution time. See [WP-196](WP-196-dashboard-net-revenue-and-paid-action-errors.md).
