...
- [ ] WP-160 — Hanko Client UI (Production Sign-In Surface for arena-client). **Drafted** 2026-05-17. Client-only WP shipping the production sign-in flow on `apps/arena-client/`: a new Pinia auth store (`stores/auth.ts`), a thin `@teamhanko/hanko-elements ^2.4.0` SDK wrapper (`auth/hankoClient.ts`, single-file broker-confined per F-2 extended to client), a new `LoginPage.vue` at the new `?route=login` route discriminator, an extended App.vue with route-guard logic for `me` + `admin-billing`, and a cutover of `MyProfilePage.vue`'s `readAuthToken()` from `localStorage.getItem('authToken')` placeholder to `useAuthStore().token`. Sign-out clears broker session + Pinia store + navigates to lobby. New env var `VITE_HANKO_TENANT_BASE_URL` (mirrors server `HANKO_TENANT_BASE_URL`). Closes the WP-099 → WP-112 → WP-126 → WP-131 stack at the client boundary; first end-to-end authenticated path on production. Unblocks the deployed-but-blocked authenticated WPs: WP-101 / WP-104 / WP-106 / WP-108 / WP-132 / WP-133 + the future WP-159/WP-107 admin-session cutover. Hard-deps: WP-099, WP-101, WP-104, WP-106, WP-108, WP-112, WP-126, WP-131, WP-132, WP-133, WP-052, WP-090 — all ✅. 01.5 NOT INVOKED (no engine surface). 01.6 post-mortem MANDATORY (new long-lived client-side auth seam). §17 cited (Vision §3 + §11 + §14); §20 N/A (no funding surface); §21 N/A (no `apps/server/src/**` touch; no HTTP endpoint or library-only function added/modified; Auth taxonomy unchanged). D-16001..D-16011 reserved. 14 files (10 source + 4 governance). See [WP-160](WP-160-hanko-client-ui.md) + [EC-174](../execution-checklists/EC-174-hanko-client-ui.checklist.md).
