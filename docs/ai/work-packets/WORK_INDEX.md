...
- [ ] WP-183 — Scheme Twist Pattern Taxonomy for Registry Viewer — pending. Registry-viewer-only. Adds 8-pattern twist taxonomy to `cards.legendary-arena.com`: static metadata files (patterns + assignments for 191 schemes), Zod schemas, singleton-cached R2 client, filter chip ribbon, CardDetail badge, CardGrid badge, `flattenSet` enrichment, `applyQuery` filter. No engine/server changes. Hard-deps: WP-182 ✅, WP-083 ✅, WP-170 ✅. EC-210. See [WP-183](WP-183-scheme-twist-pattern-taxonomy.md) + [EC-210](../execution-checklists/EC-210-scheme-twist-pattern-taxonomy.checklist.md).
