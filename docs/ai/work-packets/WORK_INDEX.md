...
- [ ] WP-192 — Hanko JWKS Refresh Interval Parsing Guard (Server) (EC-219). Server-only, single-file surgical hardening of `apps/server/src/server.mjs`'s `tryConstructHankoVerifier()` env-parse boundary. Replaces single-step `Number(refreshIntervalRaw)` with a two-step parse + `Number.isFinite()` guard so malformed `HANKO_JWKS_REFRESH_INTERVAL_MS` shapes (typo, empty string, `'123abc'`, `'Infinity'`, etc.) collapse to `undefined` and the factory's D-12603 default-substitution branch (300 000 ms) fires — instead of `NaN` propagating into `setInterval(..., NaN)` and the WHATWG spec coercing it to 1 ms (hammering Hanko's JWKS endpoint). **Origin:** the fix was first drafted during WP-176's execution session 2026-05-24 when a production deploy log surfaced `refresh=NaNms`; parked as a stash with the note "ride along with next server.mjs touch with an EC prefix"; 7 days later no server.mjs WP has absorbed it, so WP-192 lands it under proper EC governance. Single-file surgical scope: only the parse block at approximately lines 224-234 of `server.mjs` is rewritten; `envComplete`, the dev/prod diagnostics, the masked-URL logging, the `refreshLogged` ternary, and the `createHankoSessionVerifier` call shape are byte-identical pre/post. Test addition deferred (D-19202) — would require either stubbing the factory or extracting a pure helper, both expanding scope; `// why:` comment + production log evidence carry the contract. Behavior preservation: valid numeric env values produce byte-identical output. Hard-deps: WP-131 ✅ (verifier startup guard), WP-126 ✅ (factory). D-19201..D-19202 (proposed). 5 files (1 source + 4 governance). See [WP-192](WP-192-hanko-jwks-refresh-interval-guard.md) + [EC-219](../execution-checklists/EC-219-hanko-jwks-refresh-interval-guard.checklist.md).
