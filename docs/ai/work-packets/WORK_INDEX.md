...
- [ ] WP-107 — Profile Integrity / Anti-Cheat Surface. **Drafted** 2026-05-17; **Blocked** on WP-159 execution (admin-session gate). Admin-only read surface + suspend/unsuspend actions + append-only audit log; migration 015 (adds `is_suspended` to `legendary.players`, creates `legendary.admin_actions`). Three new endpoints under `/api/admin/players/:handle/`; score-submission intake gains shared `requireUnsuspendedAccount` guard. Hard-deps: WP-052, WP-101, WP-102, WP-104, WP-112, WP-126, WP-131, WP-053, WP-105, **WP-159**. D-10701..D-10703. See [WP-107](WP-107-profile-integrity-anti-cheat-surface.md).
...
- [ ] WP-159 — Admin Session Gate (Session-Based Admin Authentication). **Drafted** 2026-05-17; EC-173 drafted same day; lint-gate self-review pending. Server-only library addition that composes WP-112's `requireAuthenticatedSession` orchestrator with a new boolean `is_admin` column on `legendary.players` (migration 014) to produce `requireAdminSession(req, options): Promise<AdminSessionResult>` returning `{ ok: true; accountId } | { ok: false; code: 'unauthorized' | 'forbidden' | 'lookup_failed' }` with deterministic static `reason` strings per §A. Isolated in `apps/server/src/auth/adminSession.ts` (single file, mirrors WP-110 `adminGate.ts` precedent). `AccountId === legendary.players.ext_id` invariant locked; strict triple-equals on `is_admin`; multi-row → `lookup_failed` (data-integrity fault); no caching; bidirectional Set drift test; `COMMENT ON COLUMN` on migration. Adds new `admin-session-required` value to the `api-endpoints.md` Auth taxonomy. Deliberately does NOT cut over WP-110's `/api/admin/billing/history` route (separate follow-up swap WP). Unblocks WP-107 (the first caller). Hard-deps: WP-052, WP-101, WP-112, WP-126, WP-131 — all complete. 01.5 NOT INVOKED. D-15901, D-15902 (reserved). 7 files. See [WP-159](WP-159-admin-session-gate.md) + [EC-173](../execution-checklists/EC-173-admin-session-gate.checklist.md).
