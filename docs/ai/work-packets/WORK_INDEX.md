...
- [ ] WP-189 — Villain Effect Vocabulary Expansion: `koHeroEachPlayer` (Engine) (EC-216). Engine-only. Adds exactly one keyword — `koHeroEachPlayer` (6th, appended at position 6) — to `VillainEffectKeyword` + `VILLAIN_EFFECT_KEYWORDS`, plus a `koHeroEachPlayer` executor dispatch branch that iterates players in ascending player-ID order and KOs one hero each via the **same shared resolver** `koHeroCurrentPlayer` uses (discard→hand, ext_id lexical; NOT VP, NOT interactive). Closes the dominant each-player-KO gap (D-18802) for the unconditional magnitude-1 subset. Conceptual twin of `koHeroCurrentPlayer`; no new machinery, just a dispatch branch. Locks the **incremental-expansion governance clause** (D-18901): each-player vocab grows keyword-by-keyword only for unconditional magnitude-1 patterns; conditional/filtered/magnitude>1 stay out of MVP. Inert until WP-190 authors markers (engine/data pair, mirrors WP-185↔187, WP-186↔188). Hard-dep: WP-185 ✅. D-18901..D-18902 (proposed). 4 files (2 modified engine src + 2 modified tests). See [WP-189](WP-189-villain-effect-koheroeachplayer-vocabulary.md) + [EC-216](../execution-checklists/EC-216-villain-effect-koheroeachplayer-vocabulary.checklist.md).
