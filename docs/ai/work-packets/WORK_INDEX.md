...
- [x] WP-148 — `legendary-arena.com` + `www` Cutover Prep — Server CORS. **Done 2026-05-11 (`EC-151:` Commit A `6a4276c`; SPEC governance close in companion commit).** Direct sibling of WP-146 / EC-149 (the `cards.legendary-arena.com` precedent). Single-file edit at `apps/server/src/server.mjs` adding two entries (`https://legendary-arena.com` + `https://www.legendary-arena.com`) to the `Server({ origins: [...] })` literal array adjacent to the existing `https://play.legendary-arena.com` entry; **final array length: 7**. Pre-existing five entries preserved byte-identical (no removal, no rename, no reorder). Expanded `// why:` comment block enumerates the marketing-site Hugo bundle root + `www`-canonical alternate roles. No `domains.json` flip, no `render.yaml` change, no engine / registry / preplan / UI touch, no new runtime npm deps, no `pnpm-lock.yaml` change. Hard-deps: **WP-146 / EC-149** (the five-entry baseline on `main`). Engine baseline `698 / 150 / 0` UNCHANGED at execution; server baseline `250 / 184 / 66 / 0` UNCHANGED. 01.5 NOT INVOKED. 01.6 post-mortem SKIPPED — mechanical CORS allowlist addition with no design tension surfaced; D-14601 (WP-146 / EC-149) covers the dual-running retention pattern at the `legendary-arena.com` family level, so no new D-NNNNN required at WP-148 execution. §17 cited (Vision §10 / §22 / §24 indirect — CORS is admission, not content / scoring / RNG); §20 N/A (server-layer admission; no UI surfaces, no user-visible copy, no funding channels); §21 N/A (CORS allowlist edits are not §21.1 trigger surfaces; the catalog at `docs/ai/REFERENCE/api-endpoints.md` has no rows for CORS entries). NG-1..7 not crossed. Unblocks WP-149's marketing-site public-leaderboard page for cross-origin calls to `api.legendary-arena.com`. See [WP-148-legendary-arena-domain-cors-prep.md](WP-148-legendary-arena-domain-cors-prep.md) + [EC-151-legendary-arena-domain-cors-prep.checklist.md](../execution-checklists/EC-151-legendary-arena-domain-cors-prep.checklist.md). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) implicitly satisfied via the WP draft + preflight + copilot check chain. Commit prefix `EC-151:` on Commit A; `SPEC:` on Commit B.

- [x] WP-150 — Leaderboard Theme + Global Aggregation Endpoints. **Done 2026-05-11 (`EC-152:` Commit A `3ab2451`; SPEC governance close in companion commit).** Three D-150NN entries land at execution: D-15001 (themeId → scenarioKey via engine `buildScenarioKey` over `setupIntent.{schemeId, mastermindId, villainGroupIds}`), D-15002 (single deps-bundle Option (a) — extend `LeaderboardDependencies` with `getScenarioKeysForTheme?`), D-15003 (PAR-eligibility derivation via `listScenarioKeys` → per-scenario `checkParPublished` → `cs.scenario_key = ANY($1)`). Server test baseline `250 / 184 / 66 / 0` → **`263 / 197 / 66 / 0`** (+13 tests / +13 pass, suites unchanged at 31; +13 lies within the locked +10..+18 projection). Engine baseline `698 / 150 / 0` UNCHANGED. Theme mapping built at startup in `server.mjs` from the 70 `content/themes/*.json` files using the registry's `validateThemeFile` validator + the engine's `buildScenarioKey` helper; map frozen for process lifetime; missing-dir / invalid-file fail-soft (warn + skip). New envelope value `theme_not_found` (404) introduced. Layer boundary preserved: `apps/server/src/leaderboards/**` continues to import nothing from `@legendary-arena/registry`, `@legendary-arena/preplan`, `@legendary-arena/vue-sfc-loader`, or `boardgame.io`. Two new API-catalog rows for `/api/leaderboards/themes/:themeId` and `/api/leaderboards/top` (Status `Wired`, Auth `guest`, Authorizing WP `WP-150`). 01.5 NOT INVOKED. 01.6 post-mortem SKIPPED (no triggers fired — additive endpoint extension along the WP-054 + WP-115 pattern). Vision: §3 / §22 / §24 — scoring + leaderboard + replay surface; comparator + PAR-gate fail-closed preserved by reuse; NG-1..7 not crossed. See [WP-150-leaderboard-theme-and-global-aggregation-endpoints.md](WP-150-leaderboard-theme-and-global-aggregation-endpoints.md) + [EC-152-leaderboard-theme-and-global-aggregation-endpoints.checklist.md](../execution-checklists/EC-152-leaderboard-theme-and-global-aggregation-endpoints.checklist.md). Lint gate self-review PASS at draft; commit prefix `EC-152:` on Commit A; `SPEC:` on Commit B. Adds two public, anonymous, read-only HTTP endpoints under `/api/leaderboards/themes/:themeId` (theme-grouped) and `/api/leaderboards/top` (global Top-N PAR) extending the existing `apps/server/src/leaderboards/**` library + thin-Koa-adapter pattern from WP-054 / WP-115. Reuses `PublicLeaderboardEntry` (9 fields per D-5201) verbatim — sensitive-field exclusion preserved by construction. Same `Cache-Control: no-store` first-statement discipline (WP-115 v1.1 Patch 8); same locked error envelopes (`{ error: 'invalid_query', message }` / `{ error: 'internal_error' }`) plus one new envelope value `{ error: 'theme_not_found' }` (404 for unknown themeId). Same `final_score ASC, created_at ASC` comparator; same `limit` / `offset` bounds (1..100 / 0..10000). Layer-boundary preserved: `apps/server/src/leaderboards/**` imports nothing from `@legendary-arena/registry`, `@legendary-arena/game-engine`, `@legendary-arena/preplan`, `@legendary-arena/vue-sfc-loader`, `boardgame.io`, or any UI package — the themeId → scenarioKey[] mapping is built in `server.mjs` from the startup-loaded registry and injected via a new optional `getScenarioKeysForTheme?: (themeId) => readonly string[] | null` field on `LeaderboardDependencies`. **Open Decisions to lock at session start** (three `D-150NN` entries pre-drafted in EC-152 amendment): (a) themeId → scenarioKey[] mapping rule from `ThemeDefinition.setupIntent`; (b) dep-injection shape (single bundle vs new fourth-arg `themeGate`); (c) PAR-eligibility derivation for global Top-N (filter via `listScenarioKeys` then `checkParPublished`). **API catalog update mandatory** per §21 replace-whole-row semantics: two new rows; `Status: Wired`, `Auth: guest` (D-9905), `Authorizing WP: WP-150`. Auth posture: `guest` (D-9905) — no handle, no session. Hard-deps: **WP-054** (logic library on `main`), **WP-115** (route registration + `pg.Pool` bootstrap), **WP-055** (theme schema). 01.5 NOT INVOKED (no `LegendaryGameState` shape change; no move added; no phase hook; no replay-hash cascade). 01.6 post-mortem advisory at execution discretion. §17 cited (Vision §3 / §22 / §24 — scoring + leaderboards + replay surface; comparator + PAR-gate fail-closed preserved by reuse); §20 N/A (transport surface only; pure JSON responses; no UI surface, no funding copy); §21 PASS (two new catalog rows mandated). NG-1..7 not crossed. Test delta projected: **+10 to +18 tests / +2 suites** (locked precisely in EC-152 at session start before code edit). Pre-flight verdict: READY TO EXECUTE (2026-05-11). Copilot check verdict: PASS (2026-05-11). Commit prefix `EC-152:` at execution. See [WP-150-leaderboard-theme-and-global-aggregation-endpoints.md](WP-150-leaderboard-theme-and-global-aggregation-endpoints.md) + [EC-152-leaderboard-theme-and-global-aggregation-endpoints.checklist.md](../execution-checklists/EC-152-leaderboard-theme-and-global-aggregation-endpoints.checklist.md).

- [x] WP-149 — Public Leaderboard — Marketing-Site Hugo Page. **Done 2026-05-14 (marketing-repo commit `045fe87` on `legendary-arena-website`; engine-repo SPEC governance close in companion commit).** Cross-repo: marketing repo `C:\www\legendary-arena-com\` (Hugo + PaperMod). Three files shipped: `content/leaderboard/_index.md`, `layouts/leaderboard/list.html`, `assets/js/leaderboard.js`. Read-only `/leaderboard/` page rendering three views: (a) Top-N global PAR (default, consumes WP-150 `/api/leaderboards/top`), (b) theme-grouped via `?themeId=<kebab-case-id>` (consumes WP-150 `/api/leaderboards/themes/:themeId`), (c) scheme-mastermind placeholder via `?view=scheme-mastermind` (URL contract reserved, no v1 implementation). Client-side `fetch()` with `AbortController`; WP-115 pagination bounds (1..100 limit, 0..10000 offset); `pushState` view switching; accessible table markup (`<thead>` + `<tbody>` + `<th scope="col">`); `data-pagefind-ignore` to exclude dynamic scores from Pagefind indexing. No engine-repo source touch. No new runtime npm deps. No funding affordance in v1 (WP-097 / D-9701 / D-9801; G-1..G-7 PASS). 01.5 NOT INVOKED. 01.6 post-mortem SKIPPED. §17 cited (Vision §3, §10, §22, §24); §20 PASS; §21 N/A. NG-1..7 not crossed. Originally drafted 2026-05-11; BLOCKED on WP-148 + WP-150; lint gate self-review PASS (21 sections clean, §17 / §20 / §21 dispositioned).** Cross-repo work: implementation lives in marketing repo `C:\www\legendary-arena-com\` (Hugo + PaperMod theme at `https://www.legendary-arena.com/`); WP + EC files governed in this engine-repo ledger because the surfaces it consumes (`api.legendary-arena.com/api/leaderboards/*`) are governed here. Adds a new content path `/leaderboard/` rendering three views: **(a) Top-N global PAR** — default view, consumes WP-150 `/api/leaderboards/top`; **(b) theme-grouped** via `?themeId=<kebab-case-id>` — consumes WP-150 `/api/leaderboards/themes/:themeId`; **(c) future-tense scheme-mastermind** via `?view=scheme-mastermind` — URL contract reserved, no v1 implementation, placeholder copy locked at session start. Client-side `fetch()` only; no localStorage / IndexedDB caching of scores (page is fully stateless); no client-side score derivation; full-sentence error messages on every failure path; respects WP-115 pagination bounds (`limit` 1..100, `offset` 0..10000). Hard-deps: **WP-148 (CORS for `legendary-arena.com` + `www`)** + **WP-150 (themes + top endpoints)** + WP-055 (themeId schema). 01.5 NOT INVOKED (no engine-repo source touch; cross-repo execution-time diff against this repo is empty except for the three governance-close ledger files). 01.6 post-mortem advisory at execution discretion. §17 cited (Vision §3 / §10 / §22 / §24 — first user-visible public-leaderboard surface; the page renders existing API output without re-derivation); §20 PASS (funding-adjacent surface — public leaderboard is a §20.1 trigger; G-1..G-7 disposition complete in WP body with **zero funding affordances shipped in v1**, no donate copy, no persuasive ranking copy, no tournament-funding integration; WP-097 / D-9701 / D-9801 cited); §21 N/A (no engine-repo HTTP endpoints added / modified / removed; consumed endpoints are catalogued by their authorizing WPs WP-054 + WP-150). NG-1..7 not crossed. Pre-flight verdict: **NOT READY (BLOCKED on WP-148 + WP-150)** (2026-05-11) — landing as a [ ] placeholder row per workflow §Blocking drafts; flips to READY once both hard-deps are Done on `main`. Copilot check: SUSPENDED until pre-flight clears (procedural). Commit prefix `EC-153:` at execution (marketing-repo Commit A + engine-repo SPEC governance close). See [WP-149-public-leaderboard-marketing-page.md](WP-149-public-leaderboard-marketing-page.md) + [EC-153-public-leaderboard-marketing-page.checklist.md](../execution-checklists/EC-153-public-leaderboard-marketing-page.checklist.md).

- [x] WP-147 — PhysicalCard `companionSlug` + Physical-Side Order. **Done 2026-05-10 (`EC-150:` Commit A `adf62db`; SPEC draft `73e031c`; SPEC governance close in companion commit).** D-14701 + D-14702 land at execution. Registry baseline `39 / 4 / 0` → **`53 / 5 / 0`** (+14 tests / +1 suite — 4 new schema cases in `registry.smoke.test.ts`, 10 new in new `heroImageUrl.test.ts` including a determinism duplicate-call assertion). Engine baseline `698 / 150 / 0` UNCHANGED. `PhysicalCardSchema` gains optional `companionSlug?: string` (regex `^[a-z0-9-]+$`, min length 1, full-sentence Zod error); 1245 existing single-side and 39 existing two-side `physicalCards[]` validate without modification. `heroImageUrl()` + `R2_BASE_URL` relocate from `scripts/convert-cards/convert-cards-v15.mjs` into new `packages/registry/src/heroImageUrl.ts`; convert script imports via `../../packages/registry/dist/heroImageUrl.js`; all 6 call sites pass 4 positional arguments (explicit `undefined` when no companion). `synthesizePhysicalCards()` propagates patch-entry `companionSlug` into both the generated imageUrl and the output JSON. Drax in `mgtg.json` is the first registry application: p1 = `sides:["remove-his-spine","also-illegal"], companionSlug:"rhomann-dey"`; p3 = `sides:["i-am-invisible","xandar-is-invincible"], companionSlug:"irani-rael"`. p2/p4/p5/p6 byte-identical to pre-WP; no other hero in `mgtg.json` touched; no other set's JSON touched. **Bit-identity invariant preserved** for all 1245 single-side + 39 two-side existing `imageUrl` values (current `sides[]` arrays are alphabetical by construction; audit of physical-side correctness for the 37 non-Drax two-side entries queued as separate follow-up WP). 01.5 NOT INVOKED. 01.6 post-mortem SKIPPED (no triggers fired — additive optional schema field, narrowly-scoped DECISIONS override, single hero data fix). **Pre-session governance bundle gap** documented mid-execution: WP + EC + index rows existed in main's working tree but not on any branch reachable from this worktree; user authorized `SPEC: draft WP-147 + EC-150` commit (`73e031c`) on the execution branch to satisfy the EC-### commit-msg hook gate. WP-146 Done row + EC-149 row preserved byte-identical during the SPEC: draft (insertion above existing rows rather than overwrite — corrected an unintentional overwrite in main's uncommitted state). Vision: §2 (Content Authenticity strengthened — printed card with hero + named non-hero companion can now be expressed), §10a (Registry Viewer — imageUrl is the data hook the viewer consumes); NG-1..7 not crossed. See [WP-147-physical-card-companion-slug-and-side-order.md](WP-147-physical-card-companion-slug-and-side-order.md) + [EC-150-physical-card-companion-slug-and-side-order.checklist.md](../execution-checklists/EC-150-physical-card-companion-slug-and-side-order.checklist.md). Lint gate self-review PASS at draft; commit prefix `EC-150:` on Commit A; `SPEC:` on Commit B. Original drafted body retained as archive line below.

- [ ] (original draft body retained for archival reference; superseded by completion line above) WP-147 (drafted) — PhysicalCard `companionSlug` + Physical-Side Order. **Drafted 2026-05-10; not yet executed; lint gate self-review PASS (21 sections clean, including §17 Vision Alignment with clauses cited and §20/§21 N/A justifications); copilot check returned APPROVED — READY TO EXECUTE with surgical edits applied inline.** Extends the WP-138 `physicalCards[]` surface (D-13801/13802/13803) with a per-physical-card `companionSlug?: string` optional field so the printed artwork's named non-hero companion character (e.g., Drax's Irani Rael / Rhomann Dey on the cost-4 + cost-6 split cards in `mgtg`) appears between heroSlug and side slugs in the canonical `imageUrl` filename. Two DECISIONS land at execution: **D-14701** introduces `companionSlug` (Zod `.optional()`, regex `^[a-z0-9-]+$`, full-sentence error message; Drax/Irani Rael + Drax/Rhomann Dey is the first application; future split or companion cards across the registry follow the same pattern); **D-14702** overrides D-13802's `Array.prototype.sort()` lock **narrowly for `sides.length === 2`** in favor of source-data physical-side order (side A on the left/top of the printed card is first in the array). D-14702 is scoped tightly: D-13802's sort lock remains in effect for single-side filenames and any future automatic ordering operation; the override applies only to two-side cards where the JSON's `sides[]` array is authoritative (determinism preserved because the JSON file is identical across environments, replacing mechanical UTF-16 ordering with source-data ordering). **Architecture shift:** `heroImageUrl()` and `R2_BASE_URL` relocate from `scripts/convert-cards/convert-cards-v15.mjs` (local definitions deleted) into a new `packages/registry/src/heroImageUrl.ts` with proper TypeScript types + 10 `.test.ts` cases (signature, side-order preservation, companion insertion, length guards, companionSlug regex, empty-string guard, determinism duplicate-call assertion). Convert script imports both via `../../packages/registry/dist/heroImageUrl.js` (no workspace plumbing — operational contract already requires `pnpm --filter @legendary-arena/registry build` to have run; native Node module-resolution error is the operator preflight signal, backed by Verification Step 1b `Test-Path`). **Locked literal values for the Drax fix** (mgtg.json hero.slug stays `"drax"`, hero.name stays `"Drax"` — companion is per-physicalCard, NOT a hero pairing): p1 becomes `{ id:"p1", count:2, sides:["remove-his-spine","also-illegal"], companionSlug:"rhomann-dey", imageUrl:"…mgtg/mgtg-hr-drax-rhomann-dey-remove-his-spine-also-illegal.webp" }`; p3 becomes `{ id:"p3", count:3, sides:["i-am-invisible","xandar-is-invincible"], companionSlug:"irani-rael", imageUrl:"…mgtg/mgtg-hr-drax-irani-rael-i-am-invisible-xandar-is-invincible.webp" }`. p2/p4/p5/p6 unchanged; no other hero in `mgtg.json` touched; no other set's JSON touched. **Bit-identity invariant:** the 1245 existing single-side `imageUrl` values remain bit-identical after the change (single-side path semantically unchanged); the 39 existing two-side `imageUrl` values remain bit-identical because their current `sides[]` arrays are alphabetical by construction (audit of physical-side correctness for the 37 non-Drax two-side entries is **explicit out-of-scope follow-up** — separate WP). Grep-enforced invariant: every `heroImageUrl(...)` call site in `convert-cards-v15.mjs` passes four positional arguments (pass `undefined` explicitly when no companion source); Verification Step 4b detects 3-arg calls; Step 4c is a positive sanity match. Hard-deps: WP-137 (cardCounts authority), WP-138 (physicalCards introduction — D-13801/13802/13803), WP-140 (`_skipPair[]` annotation grammar — round-trip cleanly). 01.5 NOT INVOKED (engine-zero WP; no `LegendaryGameState` shape change; no move added; no phase hook; no replay-hash cascade). 01.6 post-mortem advisory at execution discretion (no triggers fire — additive optional schema field, narrowly-scoped DECISIONS override, single hero data fix). Test-count delta: **+14 tests** (+4 schema in `registry.smoke.test.ts`, +10 in new `heroImageUrl.test.ts`). Engine baseline projected UNCHANGED; registry baseline `39 / 4 / 0` → `~53 / ~5 / 0` (+14 tests / +1 suite). §17 cited per `00.3 §17.1` triggers (card data + card images — Vision §2 Content Authenticity, §10a Registry Viewer); §20 N/A (schema + script + data only — no global navigation / viewer / profile / tournament-funding surfaces); §21 N/A (no HTTP endpoints, no `apps/server/src/**` library functions). NG-1..NG-7 not crossed. Out of scope: **host migration `images.barefootbetters.com` → `images.legendary-arena.com`** (orthogonal future WP — `R2_BASE_URL` relocates here to set up that future single-point change), audit of the 37 other two-side physicalCards' physical-side correctness, R2 image renames for the two affected Drax files, schema raises on the `sides.length` ceiling. See [WP-147-physical-card-companion-slug-and-side-order.md](WP-147-physical-card-companion-slug-and-side-order.md) + [EC-150-physical-card-companion-slug-and-side-order.checklist.md](../execution-checklists/EC-150-physical-card-companion-slug-and-side-order.checklist.md). Commit prefix `EC-150:` at execution.
...
- [x] WP-141 — Physical Card Phase 2 — Engine + Viewer Consumer Migration. **Done 2026-05-14 (`EC-156:` code commit; `SPEC:` governance close).** Engine consumer migration (`buildCardDisplayData.ts` switches `card.imageUrl` reads to `getPhysicalCardForSide(...).imageUrl`; `buildHeroDeck.ts` computes deck reservoir from `physicalCards[].count` instead of summed-side `cardCounts`; the WP-137 fan-out helper `resolveHeroCardCopyCount` shifts source from cardCounts to physicalCards.count) + viewer consumer migration in `apps/registry-viewer/src/registry/shared.ts` (9 production read-sites across henchman / mastermind / scheme / villain / sidekick projection branches), `CardDetail.vue`, `CardGrid.vue`. Three new decisions landed at execution: **D-14101** (`G.heroDeck` split-hero representation — single-side ext_id stand-in using `physicalCard.sides[0]` per D-13804 Option A; per-side ext_id grammar D-13502 unchanged), **D-14102** (deck-size arithmetic source switches to `physicalCards[].count`; three fan-out sites — `buildHeroDeck.ts`, `buildCardStats` in `economy.logic.ts`, `buildCardDisplayData.ts` — read `physicalCards[]` with rarity-fallback), **D-14103** (viewer hero-card imageUrl lookup pattern — `physicalCards[]` side-to-imageUrl lookup at projection time in `shared.ts`; non-hero card types retain `card.imageUrl`). Acceptance gate per WP-138 §Migration Phasing: `grep -rn 'card\.imageUrl' packages/game-engine/src/ apps/registry-viewer/src/` returns zero matches outside test fixtures. Hard-deps: **WP-138 (Phase 1a — done)**, **WP-140 (Phase 1b — must land first per WP-138 §Migration Phasing — engine cannot trust `physicalCards[]` for split heroes until every split hero is curated and `--strict` mode exits 0)**. **01.5 IS INVOKED** at execution per `docs/ai/REFERENCE/01.5-runtime-wiring-allowance.md` — deck-reservoir size for split heroes changes (Falcon/Winter Soldier 27 → 14); `computeStateHash` inputs shift via `G.heroDeck` array length and content; replay-hash regeneration cascade fires (capture pre-edit `PRE_WP080_HASH`, run engine test post-edit, update literal IFF hash diverges; record pre/post pair in WP §Verification Steps Step 7 per WP-135 / WP-137 cascade template). 01.6 post-mortem MANDATORY (open D-13804 design question resolved at execution; replay-hash cascade fires; consumer-migration boundary pattern locked). Engine baseline projected `698 / 150 / 0` → `~710-718 / ~151-153 / 0` (+12-20 tests / +1-3 suites). Registry baseline UNCHANGED. Vision: §3 (determinism strengthened — split-hero deck composition correct end-to-end); §22 (replay faithfulness preserved across the Phase 2 engine change because per-side ext_id grammar D-13502 is unchanged; per-copy `#<copyIndex>` suffix from D-13702 layers on top); NG-1..7 not crossed. Out of scope: `HeroCardSchema.imageUrl` schema removal (Phase 3 / WP-142); R2 image rename (Phase 3 / WP-142); `physicalInstanceId` ext_id channel (separate later WP per D-13804 deferred path); replay-record `{ cardId, sidePlayed }` schema extensions (separate WP). See [WP-141-physical-card-phase-2-consumer-migration.md](WP-141-physical-card-phase-2-consumer-migration.md). Commit prefix `EC-NNN:` at execution (next free EC slot; not pre-claimed at draft).

- [x] WP-142 — Legends Snapshot Publisher. **Done 2026-05-14 (`EC-157:` Commit A; `SPEC:` governance close).** Background publisher in `apps/server` that writes public JSON leaderboard snapshots to R2 on a 5-minute cadence. Eight new files in `apps/server/src/legends/` (types, logic, publisher, scheduler, routes + 3 test files), plus modifications to `index.mjs`, `server.mjs`, `package.json`, `render.yaml`, and governance docs. No engine, registry, preplan, or UI package touch. **D-14201..D-14207 landed** (cadence, kill switch, inline-service, manifest-last, payload fields, no-auth health, archive retention). `@aws-sdk/client-s3` added to server deps for S3-compatible R2 API. Publisher runs inside `BEGIN; SET TRANSACTION READ ONLY; ... COMMIT;` for consistent snapshots. Manifest written LAST (D-14204). Health endpoint at `GET /health/legends-publisher` (guest, no auth). Server baseline `289 / 223 / 66 / 0` (+26 tests, +3 suites). Engine baseline `698 / 150 / 0` UNCHANGED; Registry baseline UNCHANGED. 01.5 NOT INVOKED — no engine state change. Vision: §1 (public scoreboard surface), §14 (explicit decisions); NG-1..7 not crossed. See [WP-142-legends-snapshot-publisher.md](WP-142-legends-snapshot-publisher.md). Commit prefix `EC-157:` on Commit A; `SPEC:` on Commit B.
