...
- [ ] WP-171 — Pile Browse Modal (Click-to-View Card Piles) (`apps/arena-client/src/components/play/**`, `apps/arena-client/src/pages/**`). **Drafted** 2026-05-22 on branch `claude/wp171-pile-browse-modal`. Adds a single generic leaf `<PileBrowseModal>` (Teleport-to-body, ESC + backdrop close, ARIA dialog, text-only card list) and wires `View all ▼` affordances on `KOPile.vue`, `MasterStrikePile.vue`, and `SchemeTwistPile.vue`; page-level modal state lives on `PlayDesktop.vue` + `PlayMobile.vue` (one instance each, no Pinia). Backed by the live `UIDisplayEntry[]` arrays WP-153 graduated from safe-skip 2026-05-16; no engine / registry / server runtime import (type-only `UIDisplayEntry` per D-16502). Out of scope: `EscapedPile.vue` (CityRow-nested; defer), `YourVictoryPile.vue` (renders inline; defer), `UIPlayerState.discardCards` (not in UIState — separate contracts WP), `OpponentVictoryModal.vue` deprecation (defer), card-image rendering, animations, scroll-snap / wheel-remap. Hard-deps: WP-128 ✅, WP-153 ✅, WP-166 ✅. Lint gate self-review: PASS (all 38 items, 0 carve-outs). See [WP-171](WP-171-pile-browse-modal.md).
