...
- [ ] WP-194 — Setup-Matrix Sweep Runner (Scheme × Mastermind, Manifest-Only) (Engine + Scripts + Reference + `.gitignore`) — pending (EC-221 Draft). First strict dependent of WP-193: layers the matrix-sweep dispatcher on top of `simulateOneGameAndCaptureMoves`. Adds `sweepSetupMatrix` in `packages/game-engine/src/simulation/sweep.runner.ts` exporting `cartesianProduct` (N-axis-generic), `SweepCellResult`, `CELL_SEED_SEPARATOR = '::cell:'`. Adds `scripts/sweep-setup-matrix.mjs` (operator CLI) that loads a base setup envelope + two axis files (schemeIds, mastermindIds), resumes from any existing manifest by `(schemeId, mastermindId)` identity key, dispatches each cell, and appends canonical-JSON success lines (or a 5-key fatal record on abort) to `sweep-output/<run-id>/manifest.jsonl` (gitignored). **No second execution path** — the sweep reuses WP-193's `simulateOneGameAndCaptureMoves` per cell; `runFixture` is NOT called (no per-cell fixture write). `simulation.runner.ts` / `runFixture.ts` / `fixtureSchema.ts` / `replay.execute.ts` zero diff. Locks D-19401..D-19403 (proposed): sweep dimensions = Scheme × Mastermind only + matrix-builder N-axis-generic + iteration order lex ascending; per-cell seed `${runSeed}::cell:${schemeId}:${mastermindId}` (literal separator `::cell:` carried as `CELL_SEED_SEPARATOR`); manifest-only output (gitignored) + fatal-record on abort. Per-cell budget = 1 seed (multi-seed = WP-195 statistical concern). One policy family across all seats (preserves D-19303). `--max-cells = 10000` default; soft stderr warning at `5000 < cellCount`; over-cap → full-sentence error before dispatch; NO `--retry-fatal` flag in v1. EMPTY_REGISTRY-via-host (mirrors WP-193 recorder precedent); cells vary by per-cell seed decorrelation. Hard-deps: WP-193 ✅, WP-036 ✅, WP-049 ✅ + T1 random policy. 9 files (1 new engine src + 1 new test + 1 new script + 1 modified `.gitignore` + 1 modified reference + 4 governance). Unblocks WP-195 (anomaly oracle). See [WP-194](WP-194-setup-matrix-sweep.md) + [EC-221](../execution-checklists/EC-221-setup-matrix-sweep.checklist.md).
