...
- [ ] WP-165 — Autoplay Status Endpoint (Server) — pending (drafted 2026-05-19, baseline `39c06c2`; pre-flight READY, copilot PASS). Adds one side-effect-free read endpoint `GET /api/match/autoplay/:matchId/status` so the WP-164 client can tell an autoplay match (200) from a normal live match (404) without a URL marker. Reuses WP-163's `getController` + `buildResponse` + `handlePlaybackRequest`; `200` → `{ ok, paused, historyLength, cursor, mode }` (no `uiState`); `404` → not-found envelope; strictly read-only. One new API-catalog row (D-11804, `Wired`/`guest`). D-16501. Hard-dep: WP-163 ✅. Paired with WP-164. 1 modified + 1 new test source + governance. See [WP-165](WP-165-autoplay-status-endpoint-server.md) + [EC-182](../execution-checklists/EC-182-autoplay-status-endpoint-server.checklist.md).

- [ ] WP-164 — Autoplay Playback Controls (Client) — BLOCKED on WP-165 (drafted 2026-05-19, baseline `39c06c2`; copilot PASS; pre-flight NOT READY until WP-165 lands). Client half of media-player playback for "Watch Bot Play": new service `autoplayPlayback.ts` (`getStatus` + six controls via `buildApiUrl`, D-16101; injects `setSnapshot` iff `uiState` truthy), new `AutoplayControls.vue` (5 buttons + pause/resume toggle; disabled matrix; REWIND affordance keyed on `isRewound = cursor < historyLength - 1` — corrected from the prep's nonexistent `mode === 'rewind'`), and `PlayDesktop.vue` mounting the bar only when `getStatus` resolves non-null (D-16501; gates out normal PvP). `mode` read directly (D-16304); live broadcast wins (D-16301); no debounce (D-16309); no `game-engine/setup` import (D-14401); `uiState.ts`/`LobbyView.vue`/`App.vue` untouched. Consumes existing decisions — no new D-entries. Hard-deps: WP-163 ✅, WP-165 (must land first), WP-161 ✅, WP-061 ✅. 3 source (2 new + 1 mod) + 2 tests + governance. See [WP-164](WP-164-autoplay-playback-client.md) + [EC-181](../execution-checklists/EC-181-autoplay-playback-client.checklist.md).
