...
- [ ] WP-182 — Scheme Twist Resolver Framework. Engine-only. Replaces per-scheme `if/else` dispatcher with data-driven `SchemeTwistConfig` + resolver registry. Ships 4 reusable resolvers (`reveal-or-punish`, `chained-reveals`, `wound-all`, `ko-from-hq`) + migrates Midtown Bank Robbery. New schemes = config entries, not code changes. Hard-deps: WP-009B ✅, WP-153 ✅, WP-179 ✅. D-18201. See [WP-182](WP-182-scheme-twist-resolver-framework.md) + [EC-209](../execution-checklists/EC-209-scheme-twist-resolver-framework.checklist.md).
