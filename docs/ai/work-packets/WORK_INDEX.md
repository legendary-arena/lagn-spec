...
- [ ] WP-184 — Card Mechanical Pattern Taxonomies for Registry Viewer (EC-211). Registry-viewer-only. Adds four mechanical-pattern taxonomies (hero: 10 patterns, villain: 8, henchman: 6, mastermind: 8) to `cards.legendary-arena.com`: 8 static metadata files, 4 per-taxonomy Zod slug enums + assignment schemas in `registry/schema.ts`, singleton-cached 8-fetch R2 client (`cardPatternsClient.ts`), generic `PatternFilter.vue` chip ribbon (generalizes WP-183's `SchemeTwistFilter.vue`), `CardDetail` + `CardGrid` badges, `flattenSet` extended with `patternAssignmentsByType?` (explicit `cardType`-keyed routing, pure), `applyQuery` extended with `mechanicalPatterns?` (single-cardType enforcement in logic), `FlatCard.mechanicalPattern?`. Hard-deps: WP-183 ✅, WP-083 ✅, WP-170 ✅. See [WP-184](WP-184-card-mechanical-pattern-taxonomies.md) + [EC-211](../execution-checklists/EC-211-card-mechanical-pattern-taxonomies.checklist.md).
