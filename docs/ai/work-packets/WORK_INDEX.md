...
- [ ] **(deferred placeholder)** WP-126 — External Authentication Integration (Hanko Session Verifier). Implements the `SessionVerifier` interface defined by WP-112 against Hanko (per WP-099 D-9901). Authorizing contracts: WP-099 §A (Session Validation Middleware policy); WP-099 §B "Hanko Wiring Module (Future Implementation WP)" (the surface this WP populates); WP-099 §C F-1..F-7 (the pre-merge gate this WP must satisfy); WP-099 D-9901..D-9905; WP-112 D-11201 (the sibling-WP architectural choice that promotes WP-099 §B's provisional "WP-1XX External Authentication Integration (Hanko)" name to **WP-126**). Locked module path per D-9904: all Hanko-specific code MUST live under `apps/server/src/auth/hanko/`. Scope (projected; finalized at draft): Hanko SDK installation (`@teamhanko/*` package + version pin — concrete package selection deferred to draft time), JWKS fetch + in-memory cache + refresh policy, JWT signature verification, OIDC claim extraction (mapping `sub` → `authProviderSub` and the federated-IdP claim → `'email' | 'google' | 'discord'` per WP-099 D-9902), tenant-URL / API-key environment variables (per WP-099 §B), `render.yaml` + `.env.example` updates. Tests: logic-pure with a fake JWKS endpoint plus signed-fixture tokens (no live Hanko tenant). Constraints (locked here; the future draft must honor): zero `'hanko'` enum value (F-1); Hanko code STAYS in `apps/server/src/auth/hanko/` only (F-2; verified by grep gate); zero `auth_provider` enum extension (F-1 + D-9902); zero gate change to existing guest-accessible routes (F-4); zero new dependency outside the `@teamhanko/*` family (F-5 — no Auth0 / Clerk / Passport / bcrypt / argon2 / scrypt). Dependencies: **WP-112 (hard-dep — provides the `SessionVerifier` interface this WP implements; WP-126 cannot draft until WP-112 ships)**; WP-099 (policy contract). Production wiring (registering `requireAuthenticatedSession(req, { verifier: hankoVerifier, accountResolver, database })` into authenticated route handlers) is a separate future request-handler WP that owns long-lived `pg.Pool` lifecycle (mirrors WP-053 / WP-054 / D-10202 ships-fail-closed-unwired precedent). **Not yet drafted** — this row exists to reserve the WP-126 slot and pin the contract origin so future readers do not re-derive the inline-vs-sibling choice. See D-11201 in `DECISIONS.md` (landed at WP-112 execution) and WP-099 §B.
