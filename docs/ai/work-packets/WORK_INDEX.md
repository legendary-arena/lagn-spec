...
- [ ] WP-176 — Admin Billing Auth Cutover (requireAdminSecret → requireAdminSession). Server-only. Cuts over `GET /api/admin/billing/history` from WP-110's shared-secret gate to WP-159's session-based gate. Deletes `adminGate.ts` + `adminGate.test.ts` (no remaining callers). Reduces Auth Taxonomy from 5 to 4 values (removes `admin-secret`). Hard-deps: WP-110, WP-159, WP-107, WP-112, WP-126, WP-131 — all ✅. D-17601..D-17603 (reserved). See [WP-176](WP-176-admin-billing-auth-cutover.md) + [EC-198](../execution-checklists/EC-198-admin-billing-auth-cutover.checklist.md).
