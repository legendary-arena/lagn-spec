...
- [ ] WP-193 — Policy-Mode Fixture Recording (Close WP-158 `--policy` Deferral) (Engine + Scripts) — pending (EC-220 Draft). Closes WP-158's explicit `--policy` deferral in `scripts/record-game-fixture.mjs`. Adds `simulateOneGameAndCaptureMoves` in `packages/game-engine/src/simulation/simulation.runner.ts` exporting `CapturedGameResult` + `CapturedOutcomeSummary` via an `onMoveDispatched?` callback side-channel over the existing per-turn loop — **no second execution path**. Recorder's `--policy` mode routes captured `ReplayMove[]` through `recordFromInput` → `runFixture` (the same path `--input` mode takes today) so `runFixture` remains the single oracle source (WP-158 §Contract + EC-172 §Guardrails — Determinism integrity preserved). `runFixture`'s public API NOT widened; `replay.execute.ts` NOT modified; `GameOutcome` stays internal; cross-path determinism proven indirectly via round-trip + outcome-equality test. Locks D-19301..D-19303 (proposed): single-oracle-path, no-lobby-moves in captured trace, seat-derived deterministic seeds (literal separator `::seat:`). One policy family across seats (heterogeneity = WP-194). Hard-deps: WP-158 ✅, WP-036 ✅, WP-049 ✅ + T1 random policy, WP-181 ✅. 8 files (1 modified engine src + 1 new test + 1 modified script + 1 modified reference + 4 governance). Unblocks WP-194 (matrix sweep) and WP-195 (anomaly oracle). See [WP-193](WP-193-policy-mode-fixture-recording.md) + [EC-220](../execution-checklists/EC-220-policy-mode-fixture-recording.checklist.md).
