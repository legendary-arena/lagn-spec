...
- [ ] WP-186 — Villain & Henchman Escape + Overrun Effects (Engine) (EC-213). Engine-only. Extends WP-185's hook table with `'onEscape'` timing. **Reworked 2026-05-28 to enrichment-first** (was: `[icon:]`/`[keyword:]` markup extraction) to match the re-specced WP-185 — the parser change is **prefix detection only** (`Escape:`/`Overrun:` → `onEscape`; Overrun: is a v1 synonym per D-18602); effects come from `[effect:]` markers authored by WP-188, read by WP-185's existing generic marker reader. New fire site inside the existing escape branch of `villainDeck.reveal.ts` (appended after `resolveEscapedBystanders`, preserving the WP-015 legacy ordering: counter → escape-pile → generic wound → bystander release → new card-specific effects). Executor (`villainEffects.execute.ts`) **unchanged** — dispatch is by hook lookup, not timing. `VILLAIN_EFFECT_KEYWORDS` unchanged at 5; `koHeroCurrentPlayer` resolution is zone-priority (discard→hand) then ext_id lexical, NOT VP-based (D-18503). v1 escape coverage is `gainWoundEachPlayer`-dominated; the dominant each-player-KO pattern safe-skips pending a future vocab expansion (D-18802). `captureBystander` under `onEscape` attaches to the escaped card now in `G.escapedPile` (D-18603). Hard-deps: WP-185 ✅, **WP-188 ✅ (escape markers landed 2026-05-29)**, WP-191 ✅ (ext_id grammar reconciliation — landed 2026-05-28); plus WP-009A/B ✅, WP-014A/B ✅, WP-015 ✅, WP-017 ✅. D-18601..D-18603 (proposed). 7 files (modifications to WP-185 outputs + reveal pipeline + tests). See [WP-186](WP-186-villain-escape-and-overrun-effects.md) + [EC-213](../execution-checklists/EC-213-villain-escape-and-overrun-effects.checklist.md).
