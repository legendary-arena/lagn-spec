...
- [ ] WP-186 — Villain & Henchman Escape + Overrun Effects (Engine) (EC-213). Engine-only. Extends WP-185's hook table with `'onEscape'` timing. Detects `Escape:` and `Overrun:` ability text at setup (Overrun: is a v1 synonym of Escape:; emits same `onEscape` timing per D-18602). New fire site inside the existing escape branch of `villainDeck.reveal.ts` (appended after `resolveEscapedBystanders`, preserving the WP-015 legacy ordering: counter → escape-pile → generic wound → bystander release → new card-specific effects). `VILLAIN_EFFECT_KEYWORDS` unchanged at 5 entries — vocabulary expansion is a future WP. `captureBystander` under `onEscape` attaches to the escaped card now in `G.escapedPile` (D-18603). Hard-dep: **WP-185 must land first**. Plus WP-009A/B ✅, WP-014A/B ✅, WP-015 ✅, WP-017 ✅. D-18601..D-18603 (proposed). 7 files (all modifications to WP-185 outputs + reveal pipeline + tests). See [WP-186](WP-186-villain-escape-and-overrun-effects.md) + [EC-213](../execution-checklists/EC-213-villain-escape-and-overrun-effects.checklist.md).
