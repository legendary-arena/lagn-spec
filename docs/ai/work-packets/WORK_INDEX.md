...
- [ ] WP-174 — First-Sign-In Auto-Provisioning (Read-or-Create Account Resolver). Server-only. Fixes D-16006 gap: `productionAccountResolver` becomes read-or-create, inserting a `legendary.players` row on no-match using Hanko JWT claims (email, displayName). Extends `VerifiedSessionClaim` (additive optional). Migration 016 adds UNIQUE on `(auth_provider, auth_provider_id)`. Race-safe `ON CONFLICT DO NOTHING`. Hard-deps: WP-052, WP-112, WP-126, WP-131, WP-160 — all ✅. D-17401..D-17406 (reserved). See [WP-174](WP-174-first-signin-auto-provisioning.md) + [EC-196](../execution-checklists/EC-196-first-signin-auto-provisioning.checklist.md).
