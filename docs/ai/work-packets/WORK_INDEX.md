...
- [ ] WP-175 — Arena Client Auth-Aware Navigation Surface. Client-only. Adds auth-aware nav element to `BrandHeader.vue`: signed-out → "Sign in" link (`?route=login`); signed-in → display name + "My profile" (`?route=me`) + "Sign out"; bootstrapping → `"..."` placeholder. New `useAuthNav` composable lazy-fetches display name from `/api/me/profile`. `isAuthBootstrapping` delivered via provide/inject (D-17501). Pinia auth store byte-identical. Hard-deps: WP-160, WP-174, WP-161 — all ✅. D-17501..D-17506 (reserved). See [WP-175](WP-175-arena-client-auth-nav.md) + [EC-197](../execution-checklists/EC-197-arena-client-auth-nav.checklist.md).
...
- [ ] WP-177 — Autoplay Rewind Requester Audience (Server). Server-only. Scopes D-16303 so rewind `uiState` is filtered for the requester's audience (player when identity headers are valid; spectator otherwise). New `resolveRequesterAudience` helper validates optional `X-Player-ID` / `X-Credentials` headers against boardgame.io match metadata; parameterizes `rewindUIState` with audience; threads audience through 3 rewind call sites. No engine change (`filterUIStateForAudience` already supports player audiences). Paired with a future client WP (sends headers from `autoplayPlayback.ts`). Hard-deps: WP-163, WP-164, WP-165 — all ✅. D-17701 (scopes D-16303). See [WP-177](WP-177-autoplay-rewind-requester-audience.md) + [EC-199](../execution-checklists/EC-199-autoplay-rewind-requester-audience.checklist.md).
