...
### WP-221 / EC-253 Executed — Theme Supplemental Setup Fields + Tips Display (2026-06-07)

**WP-221 done.** Adds four optional array fields to `ThemeSetupIntentSchema` after `heroDeckIds`: `bystanderSetIds`, `woundSetIds`, `sidekickCardIds`, `officerCardIds` — all `z.array(z.string().min(1)).default([])`. Adds `tips: z.array(z.string().min(1)).default([])` at the top level of `ThemeDefinitionSchema` after `flavorText` (D-22102). `themeSchemaVersion` stays `z.literal(2)` — additive-only optional-field extension requires no bump (D-22101). `bystanderSetIds` / `woundSetIds` use bare set abbreviations matching `abbr` in card set JSON (D-22103); `sidekickCardIds` / `officerCardIds` use `<setAbbr>/<slug>` for cross-set disambiguation (D-22104). `ThemeDetail.vue` renders a Tips section (below flavor text) and Bystanders / Wounds / Sidekicks / Officers rows in the Setup Intent block (after Hero Decks), each conditional on `array.length > 0`. `inhumans-royal-family.json` is the reference implementation: `bystanderSetIds: ["core", "xmen"]`, `woundSetIds: ["core", "cvwr"]`, `sidekickCardIds: ["cvwr/lockjaw"]`, `officerCardIds: []`, `tips` preserved byte-identical. Registry tests: 112 → 115 (+3 new tests #11–13). D-22101..D-22104 Active.

---

### WP-223 / EC-255 Executed — Hero Reveal KO-Attack Compound Executor (2026-06-07)

**WP-223 done.** Adds `'reveal-ko-attack'` to `HeroKeyword` union + `HERO_KEYWORDS` array (14 → 15; D-22301). Executor case in `heroEffects.execute.ts`: peeks `deck[0]`; if `cost === 0` (strict equality), (1) moves card to KO pile via `moveCardFromZone` + `koCard`, (2) grants `G.turnEconomy.attack += effect.magnitude` (fixed amount). Atomic: attack granted only when `moveCardFromZone` returns `found: true`. Guards: `playerZones` missing, deck empty, `topCardId` missing, `cardStats[topCardId]` missing, `G.turnEconomy` undefined, magnitude invalid (< 1) → silent no-op. `G.pendingHeroChoice` NOT touched. Extends `apply-hero-ability-markers.mjs` with `isRevealKoAttackCandidate` + `suggestRevealKoAttackToken` (routed after both `isRevealKoOrDrawCandidate` and `isRevealKoCandidate`); `isRevealKoCandidate` updated to exclude KO-attack compound lines. `assertValidToken` + `VALID_TOKEN_PATTERN` updated for `[keyword:reveal-ko-attack:N]` (N ≥ 1). Marks 1 hero card: `ssw2/dr-punisher-soldier-supreme/youre-a-slow-learner` (`[keyword:reveal-ko-attack:1]`) — sole corpus candidate across all 40 sets. Closes D-21903 item 2 (item 3, villain odd-draw, remains deferred). Engine tests: 1170 → 1177 (+7). D-22301 Active. Hard-dep: WP-222 ✅.
...
### WP-214 / EC-246 Executed — Villain Hero Capture & Dynamic Attack Resolution (Engine + Data) (2026-06-05)

**The engine now supports villain hero capture and dynamic fight-cost resolution.** `G.villainAttachedHeroes: Record<CardExtId, CardExtId[]>` is added (parallel to `attachedBystanders`); entries are deleted (not set to `[]`) when empty. Three new villain effect keywords (`captureHqHeroRightmost`, `captureHqHeroHighestCost`, `captureHqHeroLowestCost`) dispatch from `executeVillainAbilities` to pure helpers in `heroCapture.logic.ts`. `CardStatEntry` gains `fightCostMode: 'static' | 'dynamic'` and `fightCostBase: number`; the `buildCardStats` §2 villain loop parses `vAttack: "*"` → `dynamic, base=0` and `vAttack: "N+"` → `dynamic, base=N`. A new `resolveFightCost` function (in `economy.resolve.ts`) is the single authoritative fight-cost source — consumed by `fightVillain.ts` (replaces direct `cardStats[id].fightCost` read) and `uiState.build.ts` (projected into `UICityCard.fightCost`). Fight lifecycle: `awardAttachedHeroes(G, cardId, ctx.currentPlayer)` awards captured heroes to the defeating player's discard ("Gain that Hero"). Escape lifecycle: `koAttachedHeroesOnEscape(G, escapedCard)` KOs captured heroes to `G.ko`. `UICityCard` extended with `attachedHeroes: string[]` + `fightCost: number`; `UIState` extended with `villainAttachedHeroes`. Both `uiState.build.ts` and `uiState.filter.ts` updated. `VILLAIN_EFFECT_KEYWORDS` expanded from 7 to 10 (positions 8–10 = three capture keywords); `EFFECT_KEYWORD_LABELS` in `notableEvents.compose.ts` extended (PowerShell write to preserve Unicode curly quotes). v1 card-data curation: `skrull-shapeshifters` → `ambush: captureHqHeroRightmost`, `skrull-queen-veranke` → `ambush: captureHqHeroHighestCost` (2 markers in `data/cards/core.json`). Replay hash re-pinned (`PRE_WP080_HASH`: `8658f02b` → `f2c9f9ec`; sentinel fixture `finalStateHash` re-pinned). Pre-WP-214 G fixtures tolerated via `!G.villainAttachedHeroes` guards + `!fightCostMode` static-fallback in `resolveFightCost`. `pnpm -r build` exits 0; tests 1058 → 1102 (+44 net-new, well above ≥25 threshold). Two commits: `EC-246:` (implementation) + `SPEC:` (governance). **Decisions: D-21401..D-21405** (state shape, dynamic fight cost grammar, selector semantics, hero lifecycle, card-data markers).
...
### WP-211 / EC-245 Executed — Cross-App Sweep Type Drift Test (Dashboard ↔ Server) (2026-06-04)

**The dashboard ↔ server `SweepRunSummary` lock-step is now guarded by a test, not just a reviewer's eyes.** WP-209 (server `apps/server/src/sweep/sweep.types.ts`) and WP-210 (dashboard `apps/dashboard/src/types/sweep.ts`) declare structurally-identical `SweepRunSummary` interfaces, but the dashboard cannot import the server package (layer boundary), so there was no compile-time link — drift on either side would have been silent. WP-211 adds the missing guard: one new `node:test` file `apps/dashboard/src/types/sweep.drift.test.ts` that compares the dashboard type's runtime field set against a committed, server-derived field-set constant (the 5 names `runId`, `submittedAt`, `startedAt`, `cellCount`, `anomalyCounts`; hand-derived from the server source at baseline `8fb8f69`, NOT imported). It mirrors the within-app `opsTaxonomy.test.ts` / WP-203 drift-guard pattern, fulfilling the forward-reference both type headers already carried (surfaced as WP-210/EC-242 copilot Finding #4). **Dashboard side captured via a fully-typed `const sample: SweepRunSummary` literal** so `vue-tsc` rejects a missing/excess field at compile time under `strict` + `exactOptionalPropertyTypes`; the runtime key set is derived from `Object.keys(sample)`, never a second hand-written list. Four `node:test` assertions: sorted-keys `deepEqual` vs the committed constant, field count === 5, per-field membership (full-sentence failure messages naming BOTH `sweep.ts` and `sweep.types.ts` so the next maintainer reconciles both), and `manifestBlob`-absence (the server `SweepRunPayload` carries it; the summary excludes it). The committed constant's `// why:` provenance comment cites the server source path, baseline commit, and the **D-20703** `anomalyCounts` value-type deviation (widened to opaque `string` — a value-type difference, explicitly out of scope for this field-set guard). Guards the existing D-20703 lock — **no new DECISIONS entry**. Zero `apps/server` / `@legendary-arena/*` imports (the committed constant is the only bridge; verified — the only forbidden-token occurrences are the mandated provenance comment + error-message strings, not import statements). One `EC-245:` implementation commit (`057af88`, the only production-tree change) + one `SPEC:` governance close; 4 files (1 test + 3 governance). `pnpm --filter @legendary-arena/dashboard test` 181 → 185 (+4, prior suite count preserved); `typecheck` exit 0.

### WP-213 / EC-244 Executed — Registry Viewer `devLog` Category Derived From a `LOG_CATEGORIES` Single-Source Array (2026-06-04)

**The `devLog` `Category` union is now single-source-derived — adding a new dev-log domain costs one array element, not a hand-edited union line.** WP-208 shipped the urgent CI unblock (the 9-member `Category` literal union) under the existing D-12501 §7 closed-union convention; WP-213 changes the *mechanism* so the convention stops costing a WP each time. In `apps/registry-viewer/src/lib/devLog.ts` the hand-written union is replaced by a module-local `const LOG_CATEGORIES = [ "registry", "theme", "filter", "render", "glossary", "cardTypes", "cardAbilities", "cardPatterns", "schemeTwist" ] as const` (9 literals, locked canonical order = the extension timeline) and `type Category = (typeof LOG_CATEGORIES)[number];`. After this, a new `devLog`-consuming domain appends ONE element and the union widens automatically — it can never drift from the array because it *is* the array. **No behavior change:** the resolved `Category` type is identical pre/post, proven by both consumer clients (`cardPatternsClient.ts` 7 sites, `schemeTwistClient.ts` 8 sites) continuing to typecheck byte-identically. `devLog`'s signature/body/JSDoc and the module import are untouched (source diff confined to the audit-trail comment + the type-declaration region; word-diff shows only the union/comment swap). `LOG_CATEGORIES` + `Category` stay module-local (no `export`); the mandatory `as const` is verified present (without it the type collapses to `string` silently). No drift-detection test added — the union derives from the array via `typeof[number]`, so divergence is structurally impossible (a drift test would assert a tautology); `LOG_CATEGORIES` is NOT registered in the canonical-readonly-array list (that governs the engine's separate-union+drift pattern, not this single-source-derived one). `pnpm --filter registry-viewer typecheck` + `build` both exit 0. One `EC-244:` implementation commit (source + DECISIONS) + one `SPEC:` governance close; 5 files (1 source + 4 governance incl. DECISIONS). **Decision: D-21001** (amends D-12501 §7 — retires the manual per-domain union-extension requirement; this WP changes a recorded decision, it does not merely execute one).

### WP-208 / EC-240 Executed — Registry Viewer `devLog` Category Union Extension (2026-06-04)

**The Build Registry Viewer CI step is green again.** WP-183 added `cardPatternsClient.ts` (7 `devLog("cardPatterns", ...)` sites) + `schemeTwistClient.ts` (8 `devLog("schemeTwist", ...)` sites) but never extended the consuming `Category` literal union in `apps/registry-viewer/src/lib/devLog.ts`, so `vue-tsc --noEmit` rejected all 15 call sites with TS2345 — red on every `main` commit since the WP-183 merge. WP-208 appends `"cardPatterns"` then `"schemeTwist"` after `"cardAbilities"` (7 → 9 members, append-only) and extends the audit-trail `// why:` comment above the union to document the two new members (citing WP-183 as the mechanical-dependency origin and WP-086 / WP-125 as precedent). Purely additive type widening under the existing **D-12501 §7** closed-union lock — no new DECISIONS entry. Both client files byte-identical; `devLog` signature/body/JSDoc/imports unchanged (source diff confined to the union line + its comment). `pnpm --filter registry-viewer typecheck` + `build` both exit 0 (15 TS2345 errors resolved). One `EC-240:` implementation commit + one `SPEC:` governance close; 4 files (1 source + 3 governance). The structural follow-up — deriving the union from a single-source `LOG_CATEGORIES` array — is drafted as WP-213 / EC-244 (hard-dep'd on this WP).

### WP-212 / EC-243 Executed — Once-Per-Turn Villain Reveal Guard (Engine) (2026-06-04)

**The engine now enforces "reveal the top villain card once at the start of your turn."** `revealVillainCard` previously gated only on `G.currentStage !== 'start'`, so on `play.legendary-arena.com` a player could click reveal repeatedly within the `start` stage and draw multiple villain cards into the City. WP-212 adds an optional turn-scoped `G.villainRevealedThisTurn?: boolean` and a once-per-turn guard in the `revealVillainCard` **wrapper only** (after the stage gate, before the shared body; the flag is set unconditionally after `performVillainReveal` returns, so an exhausted-deck no-op still consumes the attempt). A second player-initiated reveal in the same turn is now a silent no-op with zero state mutation. The shared `performVillainReveal` body is unchanged, so scheme-twist chained reveals (`schemeTwistResolvers.ts`) intentionally bypass the guard. The flag resets each turn in the play phase `onBegin`. **Engine truth only** — the arena-client reveal button still gates on stage; graying it out after a reveal (a `UIState` projection of the flag) is the explicitly-deferred follow-up WP. One `EC-243:` implementation commit + one `SPEC:` governance close. Decisions: D-20901 (wrapper-only guard), D-20902 (optional-over-required; absent ⇒ false; unconditional set), D-20903 (behaviour-neutral pinned-hash re-pins + the fold-inline harness `onBegin`-mirror correction). **Execution surfaced a WP scope gap** (operator-approved fold-inline): the always-present field shifted *two* pinned hashes (sentinel fixture + `PRE_WP080_HASH`) and the replay harness `runFixture.ts` independently reimplements the `onBegin` reset and had to mirror the new flag reset — without it the sentinel fixture's `outcome` oracle wrongly changed, correctly tripping the §F hard stop. Executed file set is 11 (8 source/test/fixture + 3 governance), not the drafted 9. build 0, test 0 (1053 → 1058, +5 net-new).
