...
**How it works:** The projection script
(`C:\pcloud\BB\DEV\legendary-arena\apps\wiki-viewer\scripts\project-wiki.mjs`)
copies files from `C:\pcloud\BB\DEV\legendary-arena\ewiki\` into
`C:\pcloud\BB\DEV\legendary-arena\apps\wiki-viewer\static\ewiki\`
at build time. Hugo then copies `static/` into the build output
directory (`public/`) as-is. A file saved at
`C:\pcloud\BB\DEV\legendary-arena\ewiki\brevo-welcome-workflow.png`
becomes `https://ewiki.legendary-arena.com/ewiki/brevo-welcome-workflow.png`
after deploy.
...
`C:\pcloud\BB\DEV\legendary-arena\ewiki\`. Use lowercase kebab-case
filenames (e.g., `brevo-welcome-workflow.png`,
`brevo-campaign-setup.png`). Then reference them in markdown:

```markdown
![Brevo welcome workflow](/ewiki/brevo-welcome-workflow.png)
```
