...
const activePlayers = computed(() => {
  ...
  return data.value.find((snapshot) => snapshot.id === 'active-players') ?? null;
});
</script>
...
<template>
  <div class="widget">
    ...
    <div v-if="loading && !activePlayers" class="widget-loading">
      <div class="skeleton-block"></div>
    </div>

    <div v-else-if="error" class="widget-error">
      <p>{{ error.message }}</p>
      <button v-if="error.retryable" class="retry-button">Retry</button>
    </div>

    <div v-else-if="!activePlayers" class="widget-empty">
      <p>No player data available.</p>
    </div>

    <div v-else class="widget-data">
      <span class="metric-value">{{ formatNumber(activePlayers.value) }}</span>
      <span class="metric-label">players online now</span>
    </div>
  </div>
</template>
