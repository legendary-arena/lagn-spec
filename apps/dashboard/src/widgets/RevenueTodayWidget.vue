...
const revenueToday = computed(() => {
  ...
  return data.value.find((snapshot) => snapshot.id === 'revenue-today') ?? null;
});
</script>
...
<template>
  <div class="widget">
    ...
    <div v-if="loading && !revenueToday" class="widget-loading">
    ...
    <div v-else-if="!revenueToday" class="widget-empty">
      <p>No revenue data available.</p>
    </div>

    <div v-else class="widget-data">
      <span class="metric-value">{{ formatCurrency(revenueToday.value) }}</span>
      <span class="metric-label">earned today</span>
    </div>
  </div>
</template>
