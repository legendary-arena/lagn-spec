...
// why: each test creates its own Pinia so cross-test state cannot leak
// (the store reactive refs are owned by the active Pinia instance).
// Matches the apps/arena-client/src/stores/uiState.test.ts precedent.
...
// `isAuthenticated` as `ComputedRef<boolean>`, NOT `Ref<boolean>`. An
// attempt to write `store.isAuthenticated = true` would fail TypeScript
// at compile time (ComputedRef has no setter). The compile-time gate is
// the load-bearing check; a runtime assertion would only verify the
// absence of side effects, which is a strictly weaker guarantee.
